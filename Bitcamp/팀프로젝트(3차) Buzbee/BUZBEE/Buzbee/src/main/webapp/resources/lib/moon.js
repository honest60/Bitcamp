...
$(document).ready(function() {
	 $("#m_id").keyup(function(){
		 ...
		 if(m_id.length < 5) {
			 $('#id-check').text('아이디가 너무 짧습니다.');
		 } else if(m_id.length >= 5) {
			 $('#id-check').text('');
			 var request = $.ajax({url:"../ajax/checkId", async:false, method:"GET", data:{m_id:m_id}, dataType:"html", 
			  success: function () {},
			...
			request.done(function(data){
				if(data == 'true')  $('#id-check').text('사용할 수 없는 ID입니다.');
			});
		 }
	 });
...
function preview(input, area) {
    ...
    $(area).prop("src", URL.createObjectURL(input.files[0]));
}
...
function contentPopUp(b_no) {
	...
	request.done(function(data){
		for(var i in data) {
			var isAdmin = '<li onclick="deleteBoard('+data[i].b_no+')">게시물 삭제</li>';
			if(username != data[i].m_id) isAdmin = '<li onclick="blockUser(\''+data[i].m_id+'\', 0)">유저 차단</li>';
			
			$(".listmodal").eq(i).prop("id", "subnav-modal" + data[i].b_no);
			$("#subnav-modal" + data[i].b_no).append(isAdmin);
			$(".dropmodal").eq(i).attr("onclick", "dropMenuModal(" + data[i].b_no + ")");
			...
			if(i < data.length - 1) {
				...
				var html = '<div class="twitter-card" onclick="contentZoom('+index+')">' + 
						      ...
						      '<div class="twitter-card-icons">'+
						          ...
						          '</a>'+
						          '<li class="navi_set glyphicon glyphicon-option-horizontal topnav dropmodal" onclick="dropMenuModal('+data[i].b_no+')">'+  
								   '<ul class="subnav listmodal" id="subnav-modal">'+
								   '</ul>'+  
								'</li>'+
						      '</div>'+
						   '</div>';
...
function fetchList(startNo){
     ...
     $.ajax({
          url:baseUrl + "/ajax/board?page=" + startNo + "&newBuzzings=" + newBuzzings,
          type: "GET",
          async: false,
          dataType: "json",
			       ...
			       '<div class="media-body" onclick="contentPopUp('+vo.b_no+')">'+
			            ...
			            '<div class="twitter-card-content-board">'+
					     '<div class="twitter-card-icons-board">'+
					        ...
					        '</a>'+
					        '<li class="navi_set glyphicon glyphicon-option-horizontal topnav" onclick="dropMenu('+vo.b_no+')">'+  
							   '<ul class="subnav" id="subnav'+vo.b_no+'">'+
         			      	     isAdmin+
							   '</ul>'+  
							'</li>'+
					    '</div>'+
...
function dropMenuModal(idx) {
	if($('#subnav-modal' + idx).css("display") == 'none') {
		$('#subnav-modal' + idx).slideDown('normal').show();
	} else {
		$('#subnav-modal' + idx).slideUp('normal');
	}
